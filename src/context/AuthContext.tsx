...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (!isSupabaseConfigured) {
        ...
        const localPhone = localStorage.getItem('igo.cx.phone');
        const localRole = localStorage.getItem('igo.cx.role');
        const localAddress = localStorage.getItem('igo.cx.address');
        setUser(localEmail ? ({ 
          id: 'local-customer', 
          email: localEmail, 
          user_metadata: { 
            ...
            role: localRole || undefined,
            address: localAddress || undefined
          } 
       ...
       const handleStorage = () => {
          ...
          const nextPhone = localStorage.getItem('igo.cx.phone');
          const nextRole = localStorage.getItem('igo.cx.role');
          const nextAddress = localStorage.getItem('igo.cx.address');
          setUser(nextEmail ? ({ 
            id: 'local-customer', 
            email: nextEmail, 
            user_metadata: { 
  ...
  const updateProfile = async (metadata: { 
    ...
    if (!isSupabaseConfigured) {
      ...
      if (metadata.phone) localStorage.setItem('igo.cx.phone', metadata.phone);
      if (metadata.role) localStorage.setItem('igo.cx.role', metadata.role);
      if (metadata.address) localStorage.setItem('igo.cx.address', metadata.address);
      window.dispatchEvent(new Event('igo:customer-auth'));
      return;
    }
